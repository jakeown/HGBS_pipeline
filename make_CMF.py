...
def Cepheus_CMF_plotter(catalog_directory = '/mnt/scratch-lustre/jkeown/DS9_regions/HGBS_pipeline/'):
	...
	for i in robust_masses:
		histogram, bins = numpy.histogram(i, bins=numpy.logspace(-2.0,1.0,14))
		root_N_err = histogram**0.5
		centers = (bins[:-1]+bins[1:])/2
		print histogram
		print centers
		...
		plt.plot(centers[numpy.where(centers>0.1)], histogram[numpy.where(centers>0.1)]*factor[counter], drawstyle='steps-mid', label=regions[counter],color=colors[counter])
	...
	fig1.savefig("Cepheus_CMF_2.png")
	plt.show()
	...
	histogram, bins = numpy.histogram(Aquila_robust, bins=numpy.logspace(-2.0,1.0,14))
	root_N_err = histogram**0.5
	centers = (bins[:-1]+bins[1:])/2
	...
	plt.plot(centers[numpy.where(centers>0.1)], histogram[numpy.where(centers>0.1)], color='blue', drawstyle='steps-mid', label="Aquila")
	popt, pcov = curve_fit(func, numpy.log10(centers[numpy.where(centers>0.2)]), histogram[numpy.where(centers>0.2)], p0=guess, sigma=root_N_err[numpy.where(centers>0.2)])
	...
	plt.annotate('Peak = ' + str("{0:.2f}".format(round(10**popt[0],2))) + " M$_\odot$ $\sigma$ = " + str("{0:.2f}".format(round(popt[1],2))), xy=(0.65,0.8), xycoords=('axes fraction'), color="red")
	...
	histogram, bins = numpy.histogram(Cepheus_robust_masses, bins=numpy.logspace(-2.0,1.0,14))
	root_N_err = histogram**0.5
	centers = (bins[:-1]+bins[1:])/2
	...
	plt.plot(centers[numpy.where(centers>0.1)], histogram[numpy.where(centers>0.1)], color='green', drawstyle='steps-mid', label="Cepheus")
	#plt.hist(prestellar_robust, bins=numpy.logspace(-2.0,1.0,11), color='red')
	popt, pcov = curve_fit(func, numpy.log10(centers[numpy.where(centers>0.2)]), histogram[numpy.where(centers>0.2)], p0=guess, sigma=root_N_err[numpy.where(centers>0.2)])
	a = abs(10**(popt[0]+(pcov[0][0]**0.5)) - 10**popt[0])
	b = abs(10**(popt[0]-(pcov[0][0]**0.5)) - 10**popt[0])
	...
	plt.xlim([10**-2, 3*10**1])
	plt.ylim([1, 5*10**2])
	plt.annotate('Peak = ' + str("{0:.2f}".format(round(10**popt[0],2))) + " M$_\odot$ $\sigma$ = " + str("{0:.2f}".format(round(popt[1],2))), xy=(0.65,0.75), xycoords=('axes fraction'), color="brown")
	plt.legend()
	fig.savefig("Cepheus_CMF.png")
	plt.show()
	plt.close()
...
""", """polygon(22:30:02.398,+75:43:27.20,22:17:51.515,+75:51:18.32,22:17:15.995,+75:44:06.40,22:16:10.988,+75:42:44.32,22:17:18.087,+75:29:36.11,22:14:24.915,+75:25:09.46,22:16:41.945,+75:00:25.94,22:18:17.939,+74:49:00.15,22:19:22.504,+74:48:10.96,22:19:45.244,+74:45:16.13,22:19:53.787,+74:35:05.60,22:21:11.718,+74:35:42.29,22:26:34.567,+74:41:47.59,22:30:14.152,+74:40:38.47,22:36:40.338,+74:39:50.34,22:40:07.563,+74:34:39.84,22:41:54.559,+74:28:16.04,22:43:04.560,+74:48:07.93,22:45:24.651,+74:52:28.66,22:44:28.759,+75:14:15.62,22:44:57.653,+75:20:28.98,22:43:12.009,+75:28:16.52,22:41:08.117,+75:47:35.07,22:39:24.250,+75:45:57.68,22:37:36.747,+75:49:07.72)"""]
	...
	histogram, bins = numpy.histogram(Cepheus_combined_coldense, bins=numpy.logspace(numpy.log10(3.e20),numpy.log10(100.e21),51))
	root_N_err = histogram**0.5
	centers = (bins[:-1]+bins[1:])/2
	...
	plt.semilogx()
	plt.legend()
	fig2_1157.savefig("Cep_coldens_hist.png")
...
def bg_coldense_plotter(catalog_directory = '/mnt/scratch-lustre/jkeown/DS9_regions/HGBS_pipeline/'):
	...
	for i in prestellar_cores_bg:
		...
		plt.legend()
		fig.savefig(regions[counter] + '_cores_vs_bg_coldens.png')
		if counter==5:
			plt.show()
		counter+=1
		plt.close()
...
def core_temp_plotter(catalog_directory = '/mnt/scratch-lustre/jkeown/DS9_regions/HGBS_pipeline/'):
	...
	for i in starless_cores_temp:
		...
		plt.legend()
		fig.savefig(regions[counter] + '_cores_vs_temp.png')
		if counter==5:
			plt.show()
		counter+=1
		plt.close()
